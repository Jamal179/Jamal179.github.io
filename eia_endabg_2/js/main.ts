namespace aufgabe13 {
    ...
    function schneemannFunktion(_x: number, _y: number): void {
        let schneemann: Path2D = new Path2D();
        crc.lineWidth = 3;
        crc.strokeStyle = "black";
        crc.fillStyle = "white";
        schneemann.arc(_x, _y, 50, 0, 2 * Math.PI);
        crc.stroke(schneemann);
        crc.fill(schneemann);

        let schneemannMitte: Path2D = new Path2D();
        schneemannMitte.arc(_x, _y - 80, 30, 0, 2 * Math.PI);
        crc.stroke(schneemannMitte);
        crc.fill(schneemannMitte);

        let schneemannKopf: Path2D = new Path2D();
        crc.strokeStyle = "black";
        crc.fillStyle = "white";
        crc.lineWidth = 3;
        schneemannKopf.arc(_x, _y - 122, 10, 0, 2 * Math.PI);
        crc.stroke(schneemannKopf);
        crc.fill(schneemannKopf);
    ...
    function hintergrund(): void {
        ...
        crc.fillStyle = "LemonChiffon";
        Sonne.arc(+ 700, 100, 100, 100, Math.PI, 2 * Math.PI);
        //      X/Y/RADIUS/BOGEN       /2mal pi = 360
        crc.fill(Sonne);

        let SonnenMund: Path2D = new Path2D();
        crc.fillStyle = "black";
        crc.fillRect(650, 90, 100, 4);
        crc.fill(SonnenMund);

        let wolke: Path2D = new Path2D();
        crc.beginPath();
        crc.moveTo(190, 71);
        crc.bezierCurveTo(136, 110, 133, 155, 233, 155);
        crc.bezierCurveTo(251, 170, 310, 182, 342, 152);
        crc.bezierCurveTo(420, 150, 420, 120, 390, 100);
        crc.bezierCurveTo(431, 44, 376, 38, 343, 50);
        crc.bezierCurveTo(321, 2, 255, 29, 257, 54);
        crc.bezierCurveTo(205, 4, 153, 23, 173, 83);
        crc.closePath();
        crc.lineWidth = 5;
        crc.fillStyle = "white";
        crc.fill();
        ...
        crc.fillStyle = "brown";
        crc.fillRect(300, 350, 10, 200);
        crc.fill(hausBein);

        let haus: Path2D = new Path2D();
        crc.fillStyle = "brown";
        crc.fillRect(250, 250, 100, 100);
        crc.clearRect(270, 270, 60, 60);
        crc.strokeRect(275, 275, 50, 50);
        crc.fill(haus);
        ...
        for (let i: number = 0; i < 60; i = i + 50) {
            ...
            let x: number = 675 + i;
            let augeSonne: Path2D = new Path2D();
            augeSonne.arc(x, 75, 10, 0, 2 * Math.PI);
            crc.fillStyle = "white";
            crc.strokeStyle = "black";
            crc.fill(augeSonne);
            crc.stroke(augeSonne);

            let augeInnenSonne: Path2D = new Path2D();
            augeInnenSonne.arc(x, 75, 5, 0, 2 * Math.PI);
            crc.fillStyle = "black";
            crc.fill(augeInnenSonne);
